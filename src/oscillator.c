...
void odc_oscillator_set_waveform(oscillator *osc, const char *waveform_type)
{
	...
	osc->current_waveform_name[sizeof(osc->current_waveform_name) - 1] =
		'\0';
	...
	if (strcasecmp(waveform_type, "sine") == 0) {
		...
		osc->current_waveform =
			malloc(osc->table_size * sizeof(unsigned short));
		if (osc->current_waveform) {
			...
			fprintf(stderr,
				"Failed to allocate memory for REST "
				"waveform.\n");
			osc->is_noise = 0;
			osc->current_waveform =
				odc_oscillator_generate_sine_wave(
					osc->table_size);
			if (osc->current_waveform) {
				printf("Oscillator %p fallback to SINE "
				       "waveform.\n",
				       (void *)osc);
			} else {
				fprintf(stderr,
					...
					custom_waveform *waveform)
{
	...
	for (int i = 0; i < waveform->num_samples; i++) {
		osc->current_waveform[i] =
			(unsigned short)((waveform->samples[i] + 1.0f) *
					 32767.5f);
	}
	osc->waveform_scale = 1.0f;
	osc->is_noise = 0;
...
void odc_oscillator_stop(oscillator *osc)
{
	printf("DCO stopped\n");
}
		...
		if (!playing) {
			...
			for (int i = 0; i < odc_num_custom_waveforms; i++) {
				if (strcasecmp(current_note.waveform_name,
					       odc_custom_waveforms[i].name) ==
				    0) {
					odc_oscillator_set_custom_waveform(
						osc, &odc_custom_waveforms[i]);
			...
			if (!is_custom_waveform) {
				odc_oscillator_set_waveform(
					osc, current_note.waveform_name);
			}
				  ...
				  int num_notes, volatile int *app_running)
{
	...
	pthread_create(&sequence_thread, NULL,
		       odc_oscillator_play_sequence_thread, data);
