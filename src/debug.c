					 ...
					 float *color)
{
	...
	odc_renderer_add_triangle(renderer, x1, y1, x2, y2, x3, y3,
				  screen_width, screen_height, color);
	odc_renderer_add_triangle(renderer, x1, y1, x3, y3, x4, y4,
				  screen_width, screen_height, color);
}
				       ...
				       int screen_width, int screen_height)
{
	...
	for (int i = 0; i < MAX_FRAME_TIMES; ++i) {
		...
		add_rectangle_with_triangles(renderer, x, y, barWidth,
					     barHeight, screen_width,
					     screen_height, color);
	}
}
