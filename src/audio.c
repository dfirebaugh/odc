		       ...
		       PaStreamCallbackFlags statusFlags, void *userData)
{
	...
	if (ad == NULL || ad->app_running == NULL) {
		fprintf(stderr,
			"Audio callback error: ad or app_running is NULL.\n");
		return paAbort;
	}
	...
	for (unsigned long i = 0; i < framesPerBuffer; i++) {
		...
		for (int j = 0; j < NUM_CHANNELS; j++) {
			...
			if (osc->playing) {
				...
				if (osc->is_noise) {
					osc_sample = generate_noise_sample(
						osc, envelope);
				} else {
					osc_sample = generate_waveform_sample(
						osc, envelope);
				}
...
static float calculate_adsr_envelope(oscillator *osc)
{
	...
	if (osc->release_started) {
		...
		if (release_elapsed < osc->release_time) {
			envelope = osc->sustain_level *
				(1.0f - (release_elapsed / osc->release_time));
		} else {
			envelope = 0.0f;
			osc->playing = 0;
		}
...
static float generate_waveform_sample(oscillator *osc, float envelope)
{
	...
	if (osc->t < 0.0f) {
		fprintf(stderr,
			"Warning: osc->t is negative (%f). Resetting to 0.\n",
			osc->t);
		osc->t = 0.0f;
	}
	...
	if (!osc->is_noise && osc->current_waveform != NULL) {
		...
		if (sample_index < 0 || sample_index >= osc->table_size) {
			fprintf(stderr,
				"Error: sample_index %d out of bounds (0 - %d) "
				"for oscillator %p.\n",
		...
		waveform_sample =
			((float)osc->current_waveform[sample_index] / 32767.5f -
			 1.0f);
	} else {
		if (osc->is_noise) {
...
static float generate_noise_sample(oscillator *osc, float envelope)
{
	...
	if (osc->noise_counter <= 0) {
		...
		if (osc->noise_mode == 0) {
			feedback = ((osc->lfsr & 1) ^ ((osc->lfsr >> 1) & 1))
				<< 14;
		} else {
			feedback = ((osc->lfsr & 1) ^ ((osc->lfsr >> 6) & 1))
				<< 14;
		}
...
static void handle_vibrato(oscillator *osc, float *current_freq)
{
	if (osc->effect == EFFECT_VIBRATO) {
		...
		*current_freq += vibrato_amount *
			sinf(2.0f * M_PI * vibrato_freq * osc->note_time);
	}
}
...
static int initialize_portaudio()
{
	...
	const PaHostApiInfo *hostApiInfo =
		Pa_GetHostApiInfo(Pa_GetDefaultHostApi());
	if (hostApiInfo) {
...
static int start_portodc_audio_stream(PaStream **stream, oscillator *osc)
{
	PaError err = Pa_OpenDefaultStream(stream, 0, 1, paFloat32, SAMPLE_RATE,
					   256, odc_audio_callback, osc);
	if (err != paNoError) {
		fprintf(stderr, "PortAudio error: %s\n", Pa_GetErrorText(err));
		return -1;
	}
...
int odc_audio_is_playback_active()
{
...
void *odc_audio_play_sequence_thread(void *arg)
{
	...
	while (active_channels > 0 && *(data->app_running)) {
		...
		for (int ch = 0; ch < num_channels; ch++) {
			...
			if (!playing[ch]) {
				oscillator_note current_note =
					notes[note_indices[ch]];

				pthread_mutex_lock(&osc->mutex);

				int is_custom_waveform = 0;
				for (int i = 0; i < odc_num_custom_waveforms;
				     i++) {
					if (strcasecmp(
						    current_note.waveform_name,
						    odc_custom_waveforms[i]
							    .name) == 0) {
						odc_oscillator_set_custom_waveform(
							osc,
							&odc_custom_waveforms
								[i]);
				...
				double note_elapsed_time =
					elapsed_time - note_start_times[ch];
				oscillator_note current_note =
					notes[note_indices[ch]];
				if (note_elapsed_time >=
