			       ...
			       oscillator_note ***sequences, int **num_notes)
{
	...
	while (fgets(line, sizeof(line), file)) {
		...
		if (strncasecmp(trimmed_line, "TEMPO", 5) == 0) {
			...
			parse_note_line(trimmed_line, tempo, sequences,
					*num_notes);
		}
	}
...
static void parse_define_directive(const char *line)
{
	if (odc_num_custom_waveforms >= MAX_CUSTOM_WAVEFORMS) {
		fprintf(stderr,
			"Maximum number of custom waveforms reached.\n");
		return;
	}
	...
	if (!copy_line) {
		fprintf(stderr,
			"Memory allocation failed while parsing DEFINE "
			"directive.\n");
		return;
	}
	...
	while ((token = strtok(NULL, " \t\n")) != NULL) {
		...
		if (value < 0 || value > 255) {
			fprintf(stderr,
				"Waveform value out of range (0-255): %d\n",
				value);
			continue;
		}
		if (num_samples >= capacity) {
			...
			if (!temp) {
				fprintf(stderr,
					"Memory allocation failed while "
					"parsing waveform samples.\n");
				free(samples);
				free(copy_line);
				return;
			}
	...
	if (num_samples > 0) {
		...
		printf("Defined custom waveform: %s with %d samples\n",
		       waveform->name, waveform->num_samples);
	} else {
			    ...
			    oscillator_note ***sequences, int *num_notes)
{
	...
	int items_read =
		sscanf(line, "%d %15s %15s %15s %f %f %f %f %15s %f", &channel,
		       note_name, note_length_str, waveform_name, &attack,
		       &decay, &sustain, &release, effect_name, &sweep_end);
	...
	if (sscanf(note_length_str, "%d/%d", &numerator, &denominator) == 2 &&
		...
		fprintf(stderr,
			"Invalid note length: %s. Defaulting to quarter "
			"note.\n",
			note_length_str);
	...
	oscillator_note *temp = realloc((*sequences)[channel],
					(n + 1) * sizeof(oscillator_note));
	if (!temp) {
		fprintf(stderr,
			"Memory allocation failed for channel %d notes.\n",
			channel);
		return;
	}
	...
	if (!note->waveform_name) {
		fprintf(stderr,
			"Memory allocation failed for waveform name.\n");
		return;
	}
	...
	if (effect == EFFECT_SWEEP_UP || effect == EFFECT_SWEEP_DOWN) {
		if (items_read >= 10) {
			...
			note->sweep_end = (effect == EFFECT_SWEEP_UP)
				? note->freq * 1.5f
				: note->freq * 0.5f;
		}
	}
	...
	num_notes[channel]++;
	printf("Parsed note: Channel %d, Note %s, Duration %.2f sec, Waveform "
	       "%s, "
	       "ADSR(%.2f, %.2f, %.2f, %.2f), Effect %d, Sweep End %.2f Hz\n",
	       channel, note_name, duration, waveform_name, attack, decay,
	       sustain, release, effect, note->sweep_end);
}
